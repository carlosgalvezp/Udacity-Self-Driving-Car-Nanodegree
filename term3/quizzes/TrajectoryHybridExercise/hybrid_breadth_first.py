...
def search(grid, start, goal):
    ...
    g = 0
    h = create_heuristic_manhattan(grid, goal)
    ...
    while len(opened) > 0:
        ...
        for next_state in expand(next):
            ...
            if closed[stack2][idx(x2)][idx(y2)] == 0 and grid[idx(x2)][idx(y2)] == 0:
                f2 = g2 + h[idx(x2)][idx(y2)]
                opened.append((f2, x2, y2, theta2))
...
def create_heuristic_manhattan(grid, goal):
    h = [[100000 for row in range(len(grid[0]))] for col in range(len(grid))]

    for x in range(len(grid)):
        for y in range(len(grid[0])):
            # Manhattan distance
            if grid[x][y] == 0:
                h[x][y] = abs(goal[0] -x) + abs(goal[1] - y)
